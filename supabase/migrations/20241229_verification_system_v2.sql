-- Migration: Add verification-first architecture (Fixed)
...
CREATE INDEX IF NOT EXISTS idx_reports_application_date ON reports(application_date);
CREATE INDEX IF NOT EXISTS idx_reports_auto_ghosted ON reports(auto_ghosted);
...
-- Function to calculate days since contact (used in queries)
CREATE OR REPLACE FUNCTION get_days_since_contact(
  p_application_date TIMESTAMP WITH TIME ZONE,
  p_last_contact_date TIMESTAMP WITH TIME ZONE
)
RETURNS INTEGER AS $$
BEGIN
  RETURN EXTRACT(DAY FROM (NOW() - COALESCE(p_last_contact_date, p_application_date)))::INTEGER;
END;
$$ LANGUAGE plpgsql IMMUTABLE;

-- Function to calculate days active for job postings
CREATE OR REPLACE FUNCTION get_days_active(
  p_first_seen TIMESTAMP WITH TIME ZONE,
  p_last_seen TIMESTAMP WITH TIME ZONE
)
RETURNS INTEGER AS $$
BEGIN
  RETURN EXTRACT(DAY FROM (p_last_seen - p_first_seen))::INTEGER;
END;
$$ LANGUAGE plpgsql IMMUTABLE;
...
BEGIN
  ...
  WHERE 
    ...
    AND get_days_since_contact(application_date, last_contact_date) >= 30
    AND status = 'pending';
...
BEGIN
  ...
  SELECT AVG(get_days_since_contact(application_date, last_contact_date)) INTO avg_response_days
  FROM reports
  WHERE company_id = company_uuid 
    AND is_verified = TRUE 
    AND auto_ghosted = FALSE
...
BEGIN
  ...
  WHERE 
    jp.company_id = c.id
    AND get_days_active(jp.first_seen_date, jp.last_seen_date) >= 60
